...
group='pubmed'#"inclusion"#"exclusion"

  library(ggplot2)
  source("./util.R")
  source.file.freq <-c(paste("../../result/",group,"/allData_table",sep = ""))
  source.file.st <-c(paste("../../result/",group,"/allST_sep",sep = ""))
  mkdir(paste("../../result/",group,"/ST_analysis/",sep=""))
  out.pdf.discription <- c(paste('../../result/',group,'/ST_analysis/ST_discription.pdf',sep=''))
  pdf(out.pdf.discription)
  out.table.st<-c(paste('../../result/',group,'/ST_analysis/CDE_in_ST_total',sep=''))
  
  allFreq <- read.table(source.file.freq,sep = '\t',header = T, row.names=NULL)
  allST <- read.table(source.file.st,sep='\t', row.names=NULL)
  
  id1<-allFreq[,1]
  DiseaseName<-allFreq[,2]
  CDE<-allFreq[,3]
  Freq<-allFreq[,4]
  SemanticType<-allFreq[,5]
  ...
  axis(2)
  write.table(cde.in.ST,out.table.st,quote= F,sep='\t',col.names=F)
  #
  ...
  x<-seq(1:length(ST.table.occur[,1]))
  try1 <- cbind(x,ST.table.occur)#freq
  arr1<-vector()
  arr2<-vector()
  for( i in 1:length(try1[,1])){
    append1<-vector()
    for(j in 3:length(try1)){
      val2<-names(try1)[j]
      rep2<-try1[[j]][i]
      array2<-rep(val2,floor(rep2))
      append1<-c(append1,array2)
    }
    
    val<-row.names(try1)[i]
    rep<-try1[[2]][i]
    array1<-rep(val,floor(rep))
    arr1<-c(arr1,array1)
    
    rep3<-rep-length(append1)
    append2<-c(append1,rep("common",floor(rep3)))
    arr2<-c(arr2,append2)
  }
